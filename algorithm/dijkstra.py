        ...
        if self.searching == False:
            ...
            for i in range(-1,2):
                for j in range(-1,2):
                        ...
                        if self.env.validatePosition(adjacent_point) and self.env.map[adjacent_point.x][adjacent_point.y].getState()[0] != CELL_STATE.CLOSE:
                            distance = current_distance + current_point.getAbsDistance(adjacent_point)
                            if distance<self.distances.get(adjacent_point,float('inf')):
                                ...
                                self.shortest_path[adjacent_point] = self.shortest_path[current_point] + [adjacent_point]
