...
class BFS:
    ...
    def path_extend(self, cur_path, final_point : Point, directionX, directionY):
        ...
        #todo: or or and
        elif self.env.validatePosition(relative_point) and self.env.map[relative_point.x][relative_point.y].getState()[0] != CELL_STATE.CLOSE  and self.env.map[relative_point.x][relative_point.y].getState()[0] != CELL_STATE.OPEN:
