...
class MovingPoint:
    def __init__(self, point: Point, translates: list[SimpleTranslate]) -> None:
...
class SimpleOrbit:
    ...
    def getOrbit(self) -> list[SimpleTranslate]:
        orbit: list[Translate] = []
        lp = len(self._vt)
        if lp > 1:
            for i in range(lp):
                orbit.append(
                    SimpleTranslate(self._vt[i][0], self._vt[i][1], self._vt[i][2])
                )
        return orbit
